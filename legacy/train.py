...
#for k in label_dict.keys():
	#label_dict[k][1] = label_dict[k][1][:num_words]
...
#training_arr = np.load("training_arr_small.npy")
#training_arr_labels = np.load("training_arr_labels_small.npy")
...
#del training_arr
#del training_arr_labels
#
#with open("binary_label_dict-small.pickle", 'wb') as f:
...
def lstm_data_generator():
	...
	indices = np.random.choice(317974, 200000)
	for row in current_dataframe[indices]:
		...
		label_vectors = set([i for i in label_vectors if i in label_dict_keys and occurences[i] < 3000])
...
def lstm_data_generator_test():
	...
	for row in current_dataframe[indices]:
		...
		for l in (label_dict[i] for i in label_vectors):
			yield ({'input_1':lstm_input_patent, 'input_2':l}, {'output_binary':[1], 'output_2':meaned})
		for l in non_true_vectors:
			yield ({'input_1':lstm_input_patent, 'input_2':l}, {'output_binary':[0], 'output_2':meaned})


lstm_dataset = tf.data.Dataset.from_generator(lstm_data_generator, ({'input_1':tf.float64, 'input_2':tf.float64}, {'output_binary':tf.float64}), ({'input_1':tf.TensorShape([num_words, embedding_dim]), 'input_2':tf.TensorShape([embedding_dim])}, {'output_binary':tf.TensorShape([1])}))
lstm_dataset = lstm_dataset.batch(32, drop_remainder=True).repeat().prefetch(100)
test = tf.data.Dataset.from_generator(lstm_data_generator_test,  ({'input_1':tf.float64, 'input_2':tf.float64}, {'output_binary':tf.float64, 'output_2':tf.float64}), ({'input_1':tf.TensorShape([num_words, embedding_dim]), 'input_2':tf.TensorShape([embedding_dim])}, {'output_binary':tf.TensorShape([1]), 'output_2':tf.TensorShape([embedding_dim])}))
test = test.batch(32, drop_remainder=True).repeat()
...
cp_callback = tf.keras.callbacks.ModelCheckpoint(filepath='./lstmcheckpoint7.h5',
