...
def get_sentence_vector(sentence):
    ...
    return np.vstack(ret[:200])
label_dict = {}
import pickle

chunked_label_dict = {i:[0,np.empty(shape=(200, 1000))] for i in all_labels}
vals = np.empty(shape=(6001, 200, 1000))
with open('../dataset.csv', encoding="utf8") as csvfile:
    ...
    for df in pd.read_csv('../dataset.csv',sep=',', header = None, chunksize=1):
        ...
        vals[n-chunk_no*6000, :, :] = lstm_input_patent
        for label in label_vectors:
            if label not in all_labels:
                continue
            label_dict[label] = label_dict.get(label, [])+[n-chunk_no*6000]
        n = n+1
        if(n%6000 == 0):
            print(n)
